...
def send_email(emails, title, content, type='text'):
    ...
    emails = [email for email in emails if not 'mrxcqifu' in email]
    if type != 'html':
...
def get_function_code(func):
    return '%s_%s' % (func.__name__, func.func_code.co_filename.split('xcqifu')[-1].replace('/', '_').replace('\\', '_').replace(".py", ""))
