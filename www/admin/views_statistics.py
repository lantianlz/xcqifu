...
from www.account.interface import UserBase, ExternalTokenBase, LastActiveBase


@verify_permission('')
def statistics_activity(request, template_name='pc/admin/statistics_activity.html'):
...
@verify_permission('get_active_user')
def get_active_user(request):
    ...
    objs = LastActiveBase().get_active_user(start_date, end_date)

    page_objs = page.Cpt(objs, count=20, page=page_index).info

    # 格式化json
    num = 20 * (page_index - 1)
    data = format_active(page_objs[0], num)
